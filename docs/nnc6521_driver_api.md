...
|------|------|------|
| Byte 0 | addr | 寄存器地址 |
| Byte 1 | cmd | 命令字：普通寄存器 `0x80`，波形寄存器 `0xC0` |
| Byte 2 | data | 写入数据 |
| Byte 3 | 0x00 | 固定填充字节 |
...
|------|------|------|
| Byte 0 | addr | 寄存器地址 |
| Byte 1 | cmd | 命令字：普通寄存器 `0x00`，波形寄存器 `0x40` |
| Byte 2 | data | 返回数据 |
...
| 位置 | 延时 | 说明 |
|------|------|------|
| MOSI 数据建立后 | 4× NOP (~55ns) | 数据建立时间 |
| SCLK 上升沿后 | 8× NOP | 高电平保持 + 额外裕量 |
| SCLK 下降沿后 | 4× NOP | 低电平保持 |
| CSN 拉低/拉高前后 | 各 4× NOP | 片选信号边沿裕量 |

### CHIP_EN 复位序列

```
CHIP_EN LOW → 1ms → CHIP_EN HIGH → 5ms → 就绪
```
...
`wavegen_config()` 中使用 `|= 0x1F` 设置 `ANA_ENABLE_REG` 的所有必要位：
...
|------|------|------|
| 1.0 | 2026-06-16 | 初始版本 |
| 1.1 | 2026-07-14 | LTS 版本：修复 SPI 写协议（data 在 byte2）、增加 NOP 时序延时、修复模拟使能位 |
