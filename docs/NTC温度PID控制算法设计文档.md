...
PID 参数自整定采用**继电反馈法（Relay Feedback）**，通过 bang-bang 控制产生等幅振荡，从振荡特性中提取系统参数。

基本原理：
1. 加热器以 100% 占空比加热，直到温度达到 `target + 2°C`
2. 加热器以 0% 占空比停止加热，直到温度降到 `target - 2°C`
3. 重复上述过程，产生等幅振荡
4. 测量振荡周期 Tu 和幅度 Au

### 11.2 Ziegler-Nichols 公式

从振荡特性计算临界参数：

```
临界增益 Ku = 4d / (π × a)
其中：d = 继电器幅值（50%，即加热全开/全关的半幅）
      a = 振荡幅度 Au / 2
...
1. 上位机发送自整定命令（func=0x0C），指定目标温度
2. 设备返回启动确认（status=0x00）
3. PID 控制器切换为 bang-bang 模式
4. 执行 6 个完整振荡周期
5. 测量 Tu（振荡周期）和 Au（振荡幅度）
6. 通过 Ziegler-Nichols 公式计算 Kp、Ki、Kd
7. 自动应用新的 PID 参数
8. 返回结果（status=0x01 + 参数值）

### 11.4 安全保护

- **过热保护**：温度超过 45°C 强制中止自整定
- **超时保护**：20 分钟内未完成自动中止
- **PID 安全检查**：自整定过程中原有的安全保护机制（传感器故障检测、过热关断等）仍然生效
- **错误处理**：中止时返回错误状态（status=0x02），PID 参数不变

### 11.5 参数编码

PID 参数通过协议传输时采用 `int16 × 100` 大端序编码：
- 示例：Kp=8.0 → 800 → 0x0320 → kp_high=0x03, kp_low=0x20
- 范围：-327.68 ~ +327.67
